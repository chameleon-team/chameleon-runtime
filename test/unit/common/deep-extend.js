...
const utilModulePath = resolveTestSrc('platform/common/util/deep-extend', 'wx')

const extend = require(utilModulePath).deepExtend
...
describe('deep-extend', function () {
	...
	it('Buffer is cloned', function () {
		...
		extend(a, b);
        a.value.write('bar');
        expect(a.value.toString()).to.deep.equal('bar')
		expect(b.value.toString()).to.deep.equal('foo')
	});
	...
	it('Date object is cloned', function () {
        ...
        expect(b.d.getTime()).not.to.equal(a.d.getTime())
	});
